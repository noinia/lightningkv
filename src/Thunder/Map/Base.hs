...
data KeyAndIndex v = KeyAndIndex {-# UNPACK #-} !Key
                                 {-# UNPACK #-} !Index -- ^ index into the data vector
                   deriving (Show,Eq,Generic)


-- | VEB Tree based map.
data Map dataVec v = Map (GTree VEB SV.Vector Key (KeyAndIndex v))
                         (dataVec v)
                   deriving (Generic)
...
-- -- instance Functor g => Functor (Map f g k) where
...
-- fromAscList    :: (Enum k, GV.Vector g v) => [(k,v)] -> Map SV.Vector g k v
-- fromAscList xs = fromAscListN (length xs) xs

-- -- | Given a length n and a list of key value pairs xs, builds a Map.
-- fromAscListN      :: (Enum k, GV.Vector g v) => Size -> [(k,v)] -> Map SV.Vector g k v
...
-- insert :: Enum k => k -> v -> Map f g k v -> Map f g k v
-- insert = undefined
...
-- delete :: Enum k => k -> Map f g k v -> Map f g k v
-- delete = undefined
